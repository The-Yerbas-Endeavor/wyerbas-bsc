...
const FLAT_FEE = BigInt(yerbas.toSatoshi('1'));
const DUST_THRESHOLD = BigInt(yerbas.toSatoshi('0.0001'));
const PAYOUT_NETWORK_FEE_PER_TX = BigInt(yerbas.toSatoshi('0.0001')); // Add this to network fee for each deposit / withdrawal.
...
(async function main() {
  ...
  const version = {
    ...
    yerbasVersion: await yerbas.getClientVersion()
  };
  ...
  app.post('/generateDepositAddress', createRateLimit(20, 1), asyncHandler(async (req, res) => {
    ...
    res.send(await createTimedAndSignedMessage({
      ...
      depositAddress: await yerbas.getNewAddress()
    }));
  }));
  ...
  app.post('/registerMintDepositAddress', createRateLimit(20, 1), asyncHandler(async (req, res) => {
    ...
    await acquire(async () => {
      ...
      const { address: multisigDepositAddress, redeemScript } = await yerbas.createMultisig(
        publicSettings.authorityThreshold, depositAddresses
      );
      try {
        await yerbas.importAddress(redeemScript);
      } catch (err) {
      }
  ...
  app.post('/queryMintBalance', createRateLimit(10, 10), asyncHandler(async (req, res) => {
    ...
    const depositedAmount = yerbas.toSatoshi((await yerbas.getReceivedAmountByAddress(yerbasSettings.depositConfirmations, depositAddress)).toString());
    const depositedAmountAfterTax = meetsTax(depositedAmount) ? amountAfterTax(depositedAmount) : 0n;
    const unconfirmedAmount = yerbas.toSatoshi((await yerbas.getReceivedAmountByAddress(0, depositAddress)).toString()) - depositedAmount;
  ...
  app.post('/submitWithdrawal', createRateLimit(1, 5), asyncHandler(async (req, res) => {
    ...
    await acquire(async () => {
      ...
      if (!(await yerbas.verifyAddress(burnDestination))) {
        throw new Error('Withdrawal address is not a valid Yerbas address');
      }
      ...
      acquireStats(async () => {
        if (stats === null || ((new Date()).getTime() - stats.time) >= 1000 * 60 * 10) {
          ...
          const computeDeposits = async (confirmations, output) => {
            ...
            const depositedAmounts = await yerbas.getReceivedAmountByAddresses(confirmations, depositAddresses.map((x) => x.depositAddress));
            const totalDepositedAmount = Object.values(depositedAmounts).reduce((a, b) => a + BigInt(yerbas.toSatoshi(b.toString())), 0n).toString();
            const totalApprovableTax = Object.values(depositedAmounts).reduce((a, b) => {
              const amount = BigInt(yerbas.toSatoshi(b.toString()));
              if (meetsTax(amount)) {
          ...
          await computeDeposits(yerbasSettings.depositConfirmations, stats.confirmedDeposits);
          ...
          const computeUtxos = async (changeConfirmations, depositConfirmations, output) => {
            const changeUtxos = await yerbas.listUnspent(changeConfirmations, [yerbasSettings.changeAddress]);
            const depositUtxos = await yerbas.listUnspent(depositConfirmations, depositAddresses.map((x) => x.depositAddress));
            output.totalChangeBalance = changeUtxos.reduce((a, b) => a + BigInt(yerbas.toSatoshi(b.amount.toString())), 0n).toString();
            output.totalDepositsBalance = depositUtxos.reduce((a, b) => a + BigInt(yerbas.toSatoshi(b.amount.toString())), 0n).toString();
          };
          await computeUtxos(yerbasSettings.changeConfirmations, yerbasSettings.depositConfirmations, stats.confirmedUtxos);
  ...
  const computePendingPayouts = async (processDeposits, processWithdrawals) => {
    ...
    if (processDeposits) {
      const deposited = await yerbas.listReceivedByAddress(yerbasSettings.depositConfirmations);
      ...
      for (const a of nonEmptyMintDepositAddresses) {
        const depositedAmount = yerbas.toSatoshi(deposited[a.depositAddress].amount.toString());
  ...
  const validatePayouts = async (depositTaxPayouts, withdrawalPayouts, withdrawalTaxPayouts) => {
    ...
    if (totalTax < networkFee) {
      throw new Error(`Insufficient tax to cover network fees of ${yerbas.fromSatoshi(networkFee)}`);
    }
    ...
    for (const p of depositTaxPayouts) {
      if (!(p.depositAddress in deposited)) {
        throw new Error('Yerbas address has zero balance');
      }
      if (!(p.depositAddress in depositAddresses)) {
        throw new Error('Yerbas address not registered');
      }
      const depositedAmount = yerbas.toSatoshi(deposited[p.depositAddress].amount.toString());
      if (!meetsTax(depositedAmount)) {
  ...
  const computeUnspent = async () => {
    const changeUtxos = await yerbas.listUnspent(yerbasSettings.changeConfirmations, [yerbasSettings.changeAddress]);
    const deposited = await yerbas.listReceivedByAddress(yerbasSettings.depositConfirmations);
    const nonEmptyMintDepositAddresses = (await database.getMintDepositAddresses(Object.keys(deposited)));
    const depositUtxos = await yerbas.listUnspent(yerbasSettings.depositConfirmations, nonEmptyMintDepositAddresses.map((x) => x.depositAddress));
  ...
  const computeVouts = async (depositTaxPayouts, withdrawalPayouts, withdrawalTaxPayouts, unspent) => {
    ...
    const taxPayoutPerPayee = (totalTax - networkFee) / BigInt(yerbasSettings.taxPayoutAddresses.length);
    for (const a of yerbasSettings.taxPayoutAddresses) {
      if (a in vouts) {
    ...
    // Compute change.
    const totalUnspent = unspent.reduce((a, b) => a + BigInt(yerbas.toSatoshi(b.amount.toString())), BigInt(0));
    const change = totalUnspent - totalPayout - networkFee; // Rounding errors from taxPayout / N is absorbed into change here.
    if (change < 0) {
      throw new Error('Insufficient funds');
    }
    if (change > 0) {
      if (yerbasSettings.changeAddress in vouts) {
        vouts[yerbasSettings.changeAddress] += change;
      } else {
        vouts[yerbasSettings.changeAddress] = change;
      }
    }
    ...
    for (const address of Object.keys(vouts)) {
      if (vouts[address] >= DUST_THRESHOLD) {
        voutsFinal[address] = yerbas.fromSatoshi(vouts[address].toString());
      }
    }
    ...
    return async (req, res) => {
      await acquire(async () => {
        ...
        if (approvalChain === null) {
          approvalChain = await yerbas.createRawTransaction(unspent, vouts);
        }

        // Validate utxos and payouts against transaction and sign.
        await yerbas.verifyRawTransaction(unspent, vouts, approvalChain);

        if (!test) {
          const approvalChainNext = await yerbas.signRawTransaction(approvalChain);
          ...
          await yerbas.signRawTransaction(approvalChain);
