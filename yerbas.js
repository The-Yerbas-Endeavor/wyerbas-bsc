...
const YERBAS_COOKIE_PATH = '~/.yerbascore/.cookie'.replace('~', os.homedir);
const YERBAS_PORT = 34646;
...
function getCookie() {
  const data = fs.readFileSync(YERBAS_COOKIE_PATH, 'utf-8').split(':');
...
async function callRpc(method, params) {
  ...
  const options = {
      url: "http://localhost:" + YERBAS_PORT.toString(),
      method: "post",
...
async function listReceivedByAddress(confirmations) {
  const data = await callRpc('listreceivedbyaddress', [confirmations, false, false, true]);
  const dict = {};
...
async function getReceivedAmountByAddress(confirmations, address) {
  ...
  getReceivedAmountByAddress().catch(console.log);
