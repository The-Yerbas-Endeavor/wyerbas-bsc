...
(function() {
  ...
  const yerbasSettings = JSON.parse(fs.readFileSync(`${settingsFolder}/yerbas.json`));
  ...
  const createTimedAndSignedMessage = async (x) => {
    ...
    const blockchainInfo = await yerbas.getBlockchainInfo();
    x.valYerbasHeight = blockchainInfo.blocks - yerbasSettings.syncDelayThreshold;
    x.valYerbasHash = await yerbas.getBlockHash(blockchainInfo.blocks - yerbasSettings.syncDelayThreshold);
  ...
  const validateTimedAndSignedMessage = async (x, walletAddress, discard=true) => {
    ...
    const blockchainInfo = await yerbas.getBlockchainInfo();
    if (x.data.valYerbasHeight < blockchainInfo.blocks - 2 * yerbasSettings.syncDelayThreshold) {
      throw new Error('Message expired');
    }
    if (x.data.valYerbasHash !== await yerbas.getBlockHash(x.data.valYerbasHeight)) {
  ...
  const repl = require('repl').start({ prompt: chalk.bold('wYerbas > '), eval: eval, writer: (x) => x, ignoreUndefined: true });
  ...
  const commandCallbacks = {
    ...
    yerbasDoesAHarakiri: yerbasDoesAHarakiri
  };
...
Available commands:
  ...
  ${chalk.bold('queryMintBalance <walletAddress>')}: Queries the amount of deposited Yerbas and minted wYerbas for <wallet address>.
  ${chalk.bold('createMintTransaction <walletAddress>')}: Creates a signed transaction to mint all remaining mintable wYerbas for <wallet address>, to be submitted to the smart contract.
  ${chalk.bold('queryBurnHistory <walletAddress>')}: Queries the amount of burned wYerbas and withdrawn Yerbas for <wallet address>.
  ${chalk.bold('createBurnTransaction <amount> <destination>')}: Creates a transaction to burn <amount> of wYerbas, which can be submitted for withdrawal to <destination> on the Yerbas Mainnet.
  ${chalk.bold('submitWithdrawal <walletAddress> <index>')}: Submits the <index>-th wYerbas burn for withdrawal of Yerbas for <wallet address>.
  ...
  ${chalk.bold('yerbasDoesAHarakiri <nodeIndex>')}: ${chalk.bold.red('[AUTHORITY ONLY]')} Sends a suicide signal to node <nodeIndex>.
  ${chalk.bold('yerbasDoesAHarakiri')}: ${chalk.bold.red('[AUTHORITY ONLY]')} Sends a suicide signal to all nodes.
`);
  }
  ...
  async function queryMintBalance(mintAddress) {
    for (const x of publicSettings.authorityNodes) {
      ...
      try {
        ...
        console.log(`mintedAmount: ${yerbas.fromSatoshi(result.mintedAmount)}, depositedAmount: ${yerbas.fromSatoshi(result.depositedAmount)}, unconfirmedAmount: ${yerbas.fromSatoshi(result.unconfirmedAmount)}, depositAddress: ${result.depositAddress}`);
      } catch (error) {
        if (error.response) { console.log(getStyledError(error.response.statusCode, error.response.body)); }
        else { console.log(getStyledError(null, error.message)); }
      }
    ...
    for (const x of publicSettings.authorityNodes) {
      ...
      try {
        ...
        console.log(
          ...
          `    mintNonce: ${result.mintNonce}, mintAmount: ${yerbas.fromSatoshi(result.mintAmount)} (= ${result.mintAmount} satoshi)\n` +
    ...
    for (const x of publicSettings.authorityNodes) {
      ...
      try {
        ...
        for (const i in result) {
          console.log(`    index: ${i}, amount: ${yerbas.fromSatoshi(result[i].burnAmount)}, destination: ${result[i].burnDestination}, status: ${result[i].status}`);
        }
      } catch (error) {
        if (error.response) { console.log(getStyledError(error.response.statusCode, error.response.body)); }
        else { console.log(getStyledError(null, error.message)); }
  ...
  function createBurnTransaction(amount, destination) {
    console.log(
      ...
      chalk.red.bold('  (DO NOT COPY ANY WHITE SPACES OR YOUR TRANSACTION MAY FAIL!)\n') +
      `  amount: ${yerbas.toSatoshi(amount)}\n` +
  ...
  const executePayoutsHandler = async (processDeposits, processWithdrawals, test) => {
    ...
    for (const i in publicSettings.authorityNodes) {
      ...
      console.log(`    Total deposit tax = ${yerbas.fromSatoshi(totalDepositTaxPayout)}`);
      for (const p of _depositTaxPayouts) {
        console.log(`      ${p.depositAddress} -> ${yerbas.fromSatoshi(p.amount)}`);
      }
      console.log(`    Total withdrawal = ${yerbas.fromSatoshi(totalWithdrawalPayout)}`);
      for (const p of _withdrawalPayouts) {
        console.log(`      ${p.burnDestination} -> ${yerbas.fromSatoshi(p.amount)}`);
      }
      console.log(`    Total withdrawal tax = ${yerbas.fromSatoshi(totalWithdrawalTaxPayout)}`);
    ...
    for (const i in publicSettings.authorityNodes) {
      ...
      if (unspent === null) {
        ...
        unspent = unspent.filter((x) => ((a) => a.length === 1 && yerbas.toSatoshi(a[0].amount.toString()) === yerbas.toSatoshi(x.amount.toString()))(_unspent.filter((y) => y.txid === x.txid && y.vout === x.vout)));
      }
    }
    console.log('\n');
    ...
    if (!test) {
      ...
      const hash = await yerbas.sendRawTranscation(approvalChain);
  ...
  async function consensus() {
    ...
    for (const i in stats) {
      ...
      if (stat === undefined) {
        ...
        versionFlattened.push([
          ...
          stat.version.yerbasVersion === undefined ? '' : stat.version.yerbasVersion.toString(),
    ...
    // Yerbas settings.
    const yerbasSettingsFlattened = [];
    for (const i in stats) {
      const stat = stats[i];
      if (stat === undefined) {
        yerbasSettingsFlattened.push(['UNREACHABLE' + i, '', '', '', '']);
      } else {
        try {
          yerbasSettingsFlattened.push([
            i,
            stat.yerbasSettings.changeAddress,
            stat.yerbasSettings.changeConfirmations.toString(),
            stat.yerbasSettings.depositConfirmations.toString(),
            stat.yerbasSettings.taxPayoutAddresses.join(' ')
          ]);
        } catch {
          yerbasSettingsFlattened.push([i, '', '', '', '']);
        }
      }
    }
    const yerbasSettingsHeader = [
      nodeHeader,
    ...
    const yerbasSettingsFooter = ['Consensus'].concat(Array(yerbasSettingsHeader.length - 1).fill(consensusCell));
    s += '\n\n  [Yerbas Settings]'
    s += Table(yerbasSettingsHeader, yerbasSettingsFlattened, yerbasSettingsFooter).render();
    ...
    const confirmedDepositsHeader = [
      nodeHeader,
      { alias: "Addresses" },
      { alias: "Total Deposited", formatter: satoshiFormatter, width: yerbasWidth },
      { alias: "Approvable Tax", formatter: satoshiFormatter, width: yerbasWidth },
      { alias: "Approved Tax", formatter: satoshiFormatter, width: yerbasWidth },
      { alias: "Remaining Tax", formatter: satoshiFormatter, width: yerbasWidth }
    ];
    ...
    const withdrawalHeader = [
      nodeHeader,
      { alias: "Submissions" },
      { alias: "Total Burned", formatter: satoshiFormatter, width: yerbasWidth },
      { alias: "Approvable Amount", formatter: satoshiFormatter, width: yerbasWidth },
      { alias: "Approved Amount", formatter: satoshiFormatter, width: yerbasWidth },
      { alias: "Remaining Amount", formatter: satoshiFormatter, width: yerbasWidth },
      { alias: "Approvable Tax", formatter: satoshiFormatter, width: yerbasWidth },
      { alias: "Approved Tax", formatter: satoshiFormatter, width: yerbasWidth },
      { alias: "Remaining Tax", formatter: satoshiFormatter, width: yerbasWidth }
    ];
    ...
    const confirmedUtxoHeader = [
      nodeHeader,
      { alias: "Change Balance", formatter: satoshiFormatter, width: yerbasWidth },
      { alias: "Deposits Balance", formatter: satoshiFormatter, width: yerbasWidth },
    ];
  ...
  async function yerbasDoesAHarakiri(index) {
    ...
    for (const x of (index === undefined ? publicSettings.authorityNodes : [publicSettings.authorityNodes[parseInt(index)]])) {
      process.stdout.write(`  ${getStyledAuthorityLink(x)} ${chalk.bold('->')} `);
      try {
        const result = await post(`${getAuthorityLink(x)}/yerbasDoesAHarakiri`, await createTimedAndSignedMessage({}));
        console.log('OK');
      } catch (error) {
        if (error.response) { console.log(getStyledError(error.response.statusCode, error.response.body)); }
        else { console.log(getStyledError(null, error.message)); }
